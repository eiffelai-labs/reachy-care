...
def _load_brave_api_key() -> str | None:
    # Priority 1 : config.BRAVE_API_KEY, which is overridden on the device by
    # a gitignored config_local.py. The dashboard disables the corresponding
    # settings fields so the end customer cannot accidentally edit them
    # during a live demo.
    try:
        import config as _cfg
        key = getattr(_cfg, "BRAVE_API_KEY", None)
        if key:
            return key
    except Exception:
        pass
