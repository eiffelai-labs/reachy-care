...
class CameraWorker:
    ...
    def __init__(self, reachy_mini: ReachyMini, head_tracker: Any = None) -> None:
        ...
        # Head pitch override (chess mode — independent of face tracking interpolation)
    ...
    def get_face_tracking_offsets(
        ...
        with self.face_tracking_lock:
            ...
            return (offsets[0], offsets[1], offsets[2], offsets[3], offsets[4] + self._head_pitch_override, offsets[5])

    def set_head_pitch_override(self, pitch_rad: float) -> None:
        """Set a persistent head pitch offset (e.g. chess mode). Not affected by face tracking interpolation."""
        self._head_pitch_override = pitch_rad
        logger.info('Head pitch override set to %.3f rad (%.1f°)', pitch_rad, pitch_rad * 57.3)
            ...
            try:
                ...
                if frame is not None:
                    ...
                    if self.is_head_tracking_enabled and self.head_tracker is not None:
                        # frame arrive en BGR (OpenCV native), MediaPipe attend RGB.
                        # Sans cette conversion, MediaPipe hallucine des visages (plafond, reflets)
                        # → la tête part en l'air quand on s'approche.
                        eye_center, _ = self.head_tracker.get_head_position(frame[:, :, ::-1])
