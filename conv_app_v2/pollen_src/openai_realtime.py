...
class OpenaiRealtimeHandler(AsyncStreamHandler):
    ...
    async def receive(self, frame: Tuple[int, NDArray[np.int16]]) -> None:
        ...
        # [Reachy Care] Filtre passe-haut IIR 300Hz — coupe bruit moteurs
        # Les servos génèrent du bruit <300Hz que le VAD OpenAI interprète
        # comme de la parole → "user intervention" → flush audio → mots perdus.
        try:
            import numpy as _np_hp
            _af = audio_frame.astype(_np_hp.float32)
            _cutoff = 300.0
            _rc = 1.0 / (2 * 3.141592653589793 * _cutoff)
            _dt = 1.0 / self.input_sample_rate
            _alpha = _rc / (_rc + _dt)
            if not hasattr(self, '_hp_prev_raw'):
                self._hp_prev_raw = _np_hp.float32(0.0)
                self._hp_prev_out = _np_hp.float32(0.0)
            for _i in range(len(_af)):
                _out = _alpha * (self._hp_prev_out + _af[_i] - self._hp_prev_raw)
                self._hp_prev_raw = _af[_i]
                self._hp_prev_out = _out
                _af[_i] = _out
            audio_frame = _af.astype(_np_hp.int16)
        except Exception:
            pass  # fallback : envoyer l'audio non filtré
